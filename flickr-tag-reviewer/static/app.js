...
async function loadQueue() {
  ...
  if (state.queue.length === 0) {
    $("stageInner").innerHTML =
      `<div class="empty">No photos with open suggestions for this filter. 🎉</div>`;
...
function renderPhoto() {
  const p = state.photo;
  if (!p || !p.photo_id) {
    $("stageInner").innerHTML = `<div class="empty">Photo not found.</div>`;
    return;
  }
  ...
  (p.existing_tags || []).forEach((t) => {
    ...
    if (!m) return;
    wrap.appendChild(
      makeRect(rect64Decode(m[1]), "existing", nameOf(t.raw), false),
    );
  });
  p.suggestions.forEach((s) => {
    ...
    wrap.appendChild(
      makeRect(
        rect64Decode(s.rect_hex),
        tier,
        `${s.person_name} ${fmtCos(s.cos)}`,
        false,
        s.id,
      ),
    );
  ...
  $("photoMeta").innerHTML =
    ...
    (p.flickr_url
      ? `<a href="${p.flickr_url}" target="_blank" rel="noopener">open on Flickr ↗</a>`
      : "") +
  ...
  if (!p.existing_tags || p.existing_tags.length === 0)
    ex.textContent = "none / not fetched";
  else
    ex.innerHTML = p.existing_tags
      .map(
        (t) => `<span class="tag-chip">${escapeHtml(t.raw || t.clean)}</span>`,
      )
      .join("");
...
function isOpen(s) {
  return s.status === "pending" || s.status === "duplicate";
}
function statusLabel(status) {
  return status === "duplicate" ? "possible duplicate" : status;
}
function fmtCos(c) {
  return c == null ? "" : "cos=" + Number(c).toFixed(3);
}
function nameOf(raw) {
...
async function doAction(suggId, act) {
  const { ok, data } = await api(`/api/suggestion/${suggId}/${act}`, {
    method: "POST",
  });
  if (!ok) {
...
function prevPhoto() {
  if (state.idx > 0) showPhoto(state.idx - 1);
}
...
function actOnActive(act) {
  ...
  const id =
    state.activeSugg && open.find((s) => s.id === state.activeSugg)
      ? state.activeSugg
      : open[0].id;
  doAction(id, act);
}
...
async function openAuth() {
  ...
  if (data.mock) {
    toast("Mock mode — no real auth needed");
    return;
  }
